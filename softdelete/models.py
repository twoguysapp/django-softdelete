...
def _determine_change_set(obj, create=True):
    try:
        qs = SoftDeleteRecord.objects.filter(
...
class SoftDeleteQuerySet(query.QuerySet):
    ...
    def delete(self, using='default', *args, **kwargs):
        ...
        for obj in self:
            rs, c = SoftDeleteRecord.objects.get_or_create(
                changeset=cs or _determine_change_set(obj),
                content_type=ContentType.objects.get_for_model(obj),
                object_id=obj.pk)
...
class SoftDeleteObject(models.Model):
    ...
    def _do_delete(self, changeset, related_field_instance):
        related_field_name = related_field_instance.name
    ...
    def delete(self, *args, **kwargs):
        if self.deleted_at:
            ...
            # get_all_related_objects does not detect GenericRelation fields
            # since they are many to many fields, and a part of virtual fields
            # need to get the generic relation field name in addition to related fields
            related_field_instances = filter(
                lambda item: isinstance(item, GenericRelation),
                self._meta.virtual_fields) + self._meta.get_all_related_objects()

            for related_field_instance in related_field_instances:
                self._do_delete(cs, related_field_instance)
